...
describe('SettingsModal', () => {
  ...
  test('focuses the close button and closes on Escape', () => {
    const onClose = vi.fn();
    render(
      <SettingsModal
        isOpen={true}
  ...
  test('marks selected word length button as selected', () => {
    ...
    expect(screen.getByRole('button', { name: '4' })).not.toHaveClass(
      'selected'
    );
    expect(screen.getByRole('button', { name: '6' })).not.toHaveClass(
      'selected'
    );
  });
});
